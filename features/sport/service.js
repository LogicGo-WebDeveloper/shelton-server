...
const getAllScheduleMatches = async (sport, date) => {
  const { data } = await axiosInstance.get(`/api/v1/sport/${sport}/scheduled-events/${date}`);
  return data;
};
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (res.headers['content-type'] && res.headers['content-type'].includes('application/json')) {
            ...
            console.error("Invalid content-type. Expected application/json but received:", res.headers['content-type']);
